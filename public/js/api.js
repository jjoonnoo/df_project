...
function createUser() {
    ...
    if(checkbox.checked){
        let name = $('#name').val()
        let address = $('#detailAddress').val()+$('#extraAddress').val()
        let phone = $('#phone')
        let postalCode = $('#postcode')
        let role = 0
        let usePersonalInfo = 1
        $.ajax({
            type:'POST',
            url:'/api/user/createUser',
            data:{name:name,address:address,phone:phone,postalCode:postalCode,role:role,usePersonalInfo},
